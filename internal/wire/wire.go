...
func injectPass(name string, sig *types.Signature, calls []call, set *ProviderSet, doc *ast.CommentGroup, ig *injectorGen) {
	...
	type _cyclicStructInfo struct {
		VarName      string
		FieldNameMap map[string]types.Type // key: field_name, val: type
	}
	var (
		typeVarNameMap = new(typeutil.Map) // key: type, val: var_name
		cyclicCalls    []call
	)

	tmpCalls := calls
	calls = []call{}
	for _, c := range tmpCalls {
		if c.kind == cyclicStructField {
			cyclicCalls = append(cyclicCalls, c)
		} else {
	...
	for i := range calls {
		...
		typeVarNameMap.Set(c.out, lname)
	...
	// 记录循环依赖的结构体成员，最后再进行赋值
	for _, c := range cyclicCalls {
		varName := typeVarNameMap.At(c.out).(string)
		for fieldName, fieldType := range c.fieldNameMap {
			ig.p("\t%s.%s = %s\n", varName, fieldName, typeVarNameMap.At(fieldType).(string))
		}
	}
